...
Optional for legacy OTP emails (OTP auth is currently disabled):
...
| `POST` | `/auth/register/verify` | Legacy OTP route, temporarily disabled (`410`) |
| `POST` | `/auth/login/verify` | Legacy OTP route, temporarily disabled (`410`) |
| `POST` | `/auth/otp/resend` | Legacy OTP route, temporarily disabled (`410`) |
...
- Legacy OTP endpoints return `410`: this is expected while OTP auth is temporarily disabled.
